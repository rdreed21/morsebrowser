...
export function NoiseAccordion() {
  ...
  return (
    <>
      <h2 className="accordion-header" id="headingNoise">
        <button
      ...
      <div id="collapseNoise" className={`accordion-collapse${isOpen ? ' show' : ' collapse'}`} aria-labelledby="headingNoise">
        <div className="accordion-body">
          <div className="row row-cols-1 row-cols-sm-2 gy-2 gx-2">
            <div className="col-auto">
              <fieldset className="btn-group" aria-label="Noise Type">
                <input type="radio" className="btn-check" value="off" name="btnradio" id="btnradio1"
                  autoComplete="off" checked={noiseType === 'off'} onChange={() => vm.noiseType('off')} aria-label="Off" />
                <label className="btn btn-outline-primary" htmlFor="btnradio1" aria-hidden="true">Off</label>

                <input type="radio" className="btn-check" value="white" name="btnradio" id="btnradio2"
                  autoComplete="off" checked={noiseType === 'white'} onChange={() => vm.noiseType('white')} aria-label="White" />
                <label className="btn btn-outline-info" htmlFor="btnradio2" aria-hidden="true">White</label>

                <input type="radio" className="btn-check" value="brown" name="btnradio" id="btnradio3"
                  autoComplete="off" checked={noiseType === 'brown'} onChange={() => vm.noiseType('brown')} aria-label="Brown" />
                <label className="btn btn-outline-secondary" htmlFor="btnradio3" aria-hidden="true">Brown</label>

                <input type="radio" className="btn-check" value="pink" name="btnradio" id="btnradio4"
                  autoComplete="off" checked={noiseType === 'pink'} onChange={() => vm.noiseType('pink')} aria-label="Pink" />
                <label className="btn btn-outline-danger" htmlFor="btnradio4" aria-hidden="true">Pink</label>
              </fieldset>
            </div>
            <div className="col-auto">
