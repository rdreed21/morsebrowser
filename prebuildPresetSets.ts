...
const setsDir = path.join(presetsDir, 'sets')
const contents = fs.readdirSync(setsDir, { withFileTypes: true })
const jsDir = path.join(srcDir, 'morse')
const templateJs = path.join(jsDir, 'morsePresetSetFinderTemplate.js')
const outputJs = path.join(jsDir, 'morsePresetSetFinder.js')
